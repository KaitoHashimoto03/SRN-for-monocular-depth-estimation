...
#     def forward(self, depths, depth_gt, mask):
#         d = torch.log(depths[2][mask]) - torch.log(depth_gt[mask])
#         depth_gt_2 = self.down(depth_gt)
#         mask2 = depth_gt_2 > 0.1
#         depth_gt_3 = self.down(depth_gt_2)
#         mask3 = depth_gt_3 > 0.1
#         d2 = torch.log(depths[1][mask2]) - torch.log(depth_gt_2[mask2])
#         d3 = torch.log(depths[0][mask3]) - torch.log(depth_gt_3[mask3])
#         loss = torch.sqrt((d ** 2).mean() - self.variance_focus * (d.mean() ** 2)) * 5.0
#         loss2 = torch.sqrt((d2 ** 2).mean() - self.variance_focus * (d2.mean() ** 2)) * 2.5
#         loss3 = torch.sqrt((d3 ** 2).mean() - self.variance_focus * (d3.mean() ** 2)) * 2.5
...
class DecodeModel(nn.Module):
    def __init__(self,half=False):
        ...
        self.aspp = ASPP_block(64+128,64,64)
